...
export default function Register() {
  ...
  const handleGoogleCallback = async (response) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/auth/google`, {
        method: 'POST',
  ...
  return (
    <Layout>
      <Head>
        <title>Register | Syntiant Atlas</title>
        <meta name="description" content="Create your Syntiant Atlas account and start investing in fractional real estate" />
      </Head>
      ...
      <div className="min-h-[80vh] flex items-center justify-center bg-gray-50 relative overflow-hidden py-20">
        ...
        <div className="bg-white p-8 md:p-10 rounded-2xl shadow-[0_12px_30px_0_rgba(19,21,35,0.1)] w-full max-w-lg z-10 mx-4 border border-gray-100">
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 text-dao-blue-dark p-4 rounded-lg text-sm text-center mb-6 border border-blue-100">
            ...
            <div className="flex justify-center gap-6 mb-6">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 text-dao-blue focus:ring-dao-blue"
                />
                <span className={`text-sm font-medium transition ${selectedRole === 'investor' ? 'text-dao-blue' : 'text-gray-600'}`}>
                  Investor
                </span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  ...
                  className="w-4 h-4 text-dao-blue focus:ring-dao-blue"
                />
                <span className={`text-sm font-medium transition ${selectedRole === 'seller' ? 'text-dao-blue' : 'text-gray-600'}`}>
                  Seller
                </span>
              </label>
            </div>
            ...
            >
              {isLoading ? (
                <span className="flex items-center gap-2">
                  <svg className="animate-spin h-5 w-5 text-dao-blue" viewBox="0 0 24 24">
            ...
            <div className="text-center">
              ...
              <Link href="/admin/login">
                <span className="text-dao-blue font-medium text-sm hover:underline cursor-pointer">
          ...
          <p className="mt-8 text-center text-gray-600 text-sm">
            ...
            <Link href="/login">
              <span className="text-dao-blue font-semibold cursor-pointer hover:underline">Log in</span>
            </Link>
          </p>
          ...
          <p className="mt-4 text-center text-gray-400 text-xs">
            ...
            <Link href="/terms-and-conditions">
              <span className="text-dao-blue hover:underline cursor-pointer">Terms of Service</span>
            </Link>{' '}
            and{' '}
            <Link href="/privacy-policy">
              <span className="text-dao-blue hover:underline cursor-pointer">Privacy Policy</span>
            </Link>
          </p>
        </div>
      </div>
